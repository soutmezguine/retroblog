const db = require('./db');
const { getCurrentTime, formatDate } = require('./time');
...
async function getCommonData() {
    ...
    const currentTime = await getCurrentTime(settings);

    return { settings, pages, categories, tags, archives, currentTime, formatDate: (date) => formatDate(date, settings) };
}
